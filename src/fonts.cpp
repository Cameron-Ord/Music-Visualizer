...
std::vector<Text> *SDL2Fonts::get_dir_vec() { return &dir_text_vec; }

std::vector<Text> *SDL2Fonts::get_song_vec() { return &song_text_vec; }

void SDL2Fonts::create_dir_text(const std::vector<Directory> d, SDL_Renderer *r,
                                const SDL_Color color, TTF_Font *font) {
  for (Directory dir : d) {
    Text tmp =
        create_text(dir.directory_name, font, r, dir.directory_id, color);
    if (tmp.is_valid) {
...
Text SDL2Fonts::create_text(const std::string text, TTF_Font *font,
                            SDL_Renderer *r, const int text_id,
                            const SDL_Color color) {
  Text text_entity = {.tex = NULL,
                      .surf = NULL,
                      .rect = {0, 0, 0, 0},
                      .id = text_id,
                      .is_valid = false,
