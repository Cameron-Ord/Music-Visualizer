...
ProgramFiles::ProgramFiles() {
  std::vector<std::string> gfx_file_names{ "music_icon.png", "pause_icon.png",    "play_icon.png",
                                           "seek_icon.png",  "settings_icon.png", "stop_icon.png" };

  for (std::string file_name_str : gfx_file_names) {
    Icon tmp = { .file_name = file_name_str, .texture = NULL, .surface = NULL, .is_valid = 0 };
...
bool
ProgramFiles::fill_files(std::string src_path, std::string slash) {
  return true;
}

bool
ProgramFiles::fill_directories(std::string src_path, std::string slash) {
  DIR*           directory = NULL;
  struct dirent* entry;
...
void
ProgramFiles::clear_files() {}
void
ProgramFiles::clear_directories() {}

std::vector<Files>*
ProgramFiles::retrieve_directory_files() {
  return &files;
}
std::vector<Directory>*
ProgramFiles::retrieve_directories() {
  return &directories;
}
