...
void FourierTransform::fft_func(float *in, size_t stride, std::complex<float> *out, size_t n) {
    if (n == 1) {
        out[0] = in[0];
        return;
    }
    ...
    for (size_t k = 0; k < n / 2; ++k) {
        float t = (float) k / n;
        std::complex<float> v = std::exp(std::complex<float>(0, -2.0f * M_PI * t)) * out[k + half_n];
...
void FourierTransform::fft_push(uint32_t pos, float *audio_data_buffer, int bytes) {
...
void FourierTransform::apply_smoothing() {
    ...
    for (size_t i = 0; i < data.output_len; ++i) {
        ...
        bufs.smoothed[i] += (bufs.processed[i] - bufs.smoothed[i]) * 8 * (1.0 / FPS);
    }
}
