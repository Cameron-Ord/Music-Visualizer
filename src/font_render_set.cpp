...
void SDL2Renderer::render_set_text_bg(const WIN_SIZE *sizes, const std::vector<Text> *text_vec,
...
void SDL2Renderer::render_set_text(const WIN_SIZE *sizes, std::vector<Text> *text_vec) {
    int pixel_increment = sizes->HEIGHT / (static_cast<int>(text_vec->size()) + 1);
    int pixel_accumulate = pixel_increment;
