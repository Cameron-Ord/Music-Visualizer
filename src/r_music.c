...
void render_seek_bar(const int w, const SDL_Color *col, SDL_Renderer *r) {
  ...
  float normalized = (float)get_ad()->position / get_ad()->length;
...
void render_draw_music(const int w, const int h, SDL_Renderer *r,
  ...
  if (get_data()->output_len <= 0)
    return;

  SDL_RenderSetViewport(r, NULL);
  SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);

  const size_t len = get_data()->output_len;
  const float *smear = get_bufs()->smear;
  const float *smoothed = get_bufs()->smoothed;
