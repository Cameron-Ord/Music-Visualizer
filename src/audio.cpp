...
void callback(void *data, uint8_t *stream, int len) {
    ...
    uint32_t copy = ((uint32_t) len / sizeof(float) < remaining) ? (uint32_t) len / sizeof(float) : remaining;
    ...
    userdata->fft->fft_push(*pos, userdata->ad->get_audio_data()->buffer, copy * sizeof(float));
